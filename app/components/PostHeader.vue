...
<template>
  <header class="relative overflow-hidden bg-neutral-900 text-white">
    ...
    <div class="relative mx-auto max-w-4xl px-4 pb-16 pt-32 sm:pb-24 sm:pt-48">
      <h1 class="text-3xl font-extrabold tracking-tight drop-shadow-md sm:text-5xl lg:text-6xl">{{ title }}</h1>
      <h2 v-if="subtitle" class="mt-4 text-xl font-semibold drop-shadow sm:text-2xl lg:text-3xl">{{ subtitle }}</h2>
      <p class="mt-6 text-base font-light italic sm:text-lg">
        Published on <time :datetime="date">{{ formatDate(date) }}</time><template v-if="readingTime">. Takes {{ readingTime }} to read.</template>
      </p>
    </div>
  </header>
</template>
