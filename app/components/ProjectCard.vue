...
<template>
  <article class="flex flex-col overflow-hidden rounded-sm border border-neutral-200 bg-paper dark:border-neutral-800 dark:bg-night-muted">
    <a v-if="project.image" :href="project.website || project.github || '#'" target="_blank" class="block">
    ...
    <div class="flex flex-1 flex-col p-5">
      ...
      <ul class="mt-4 flex gap-3">
        <li v-if="project.website">
          <a :href="project.website" title="Project website" target="_blank" class="inline-flex size-10 items-center justify-center rounded-full bg-ink text-white hover:bg-brand dark:bg-neutral-200 dark:text-night dark:hover:bg-brand dark:hover:text-white">
            <Icon name="lucide:link" class="size-4" aria-hidden="true" /><span class="sr-only">Website</span>
          </a>
        </li>
        <li v-if="project.github">
          <a :href="project.github" title="Project source code" target="_blank" class="inline-flex size-10 items-center justify-center rounded-full bg-ink text-white hover:bg-brand dark:bg-neutral-200 dark:text-night dark:hover:bg-brand dark:hover:text-white">
